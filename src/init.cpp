...
void Shutdown()
{
    ...
    RenameThread("ethernode-shutoff");
...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: ethernode.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: ethernoded.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeethernodeamount=<n> " + _("Keep N EtherNode anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. EtherNode is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. EtherNode is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("EtherNode version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "EtherNode server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of EtherNode") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart EtherNode to complete") << "\n";
    ...
    nAnonymizeEtherNodeAmount = GetArg("-anonymizeethernodeamount", 0);
    if(nAnonymizeEtherNodeAmount > 999999) nAnonymizeEtherNodeAmount = 999999;
    if(nAnonymizeEtherNodeAmount < 2) nAnonymizeEtherNodeAmount = 2;
    ...
    LogPrintf("Anonymize EtherNode Amount %d\n", nAnonymizeEtherNodeAmount);
