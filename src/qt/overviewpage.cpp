...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeEtherNodeAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeEtherNodeAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeEtherNodeAmount = strAnonymizeEtherNodeAmount.remove(strAnonymizeEtherNodeAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeEtherNodeAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeEtherNodeAmount*COIN) nMaxToAnonymize = nAnonymizeEtherNodeAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeEtherNodeAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeEtherNodeAmount));
        strAnonymizeEtherNodeAmount = strAnonymizeEtherNodeAmount.remove(strAnonymizeEtherNodeAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeEtherNodeAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
...
void OverviewPage::toggleDarksend(){
    ...
    if(!fEnableDarksend){
        ...
        if(nAnonymizeEtherNodeAmount == 0){
