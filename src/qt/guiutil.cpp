...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("ethernode"))
        return false;
...
{
    // Convert ethernode:// to ethernode:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("ethernode://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "ethernode:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "EtherNode.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for EtherNode.lnk
...
{
    return GetAutostartDir() / "ethernode.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a ethernode.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=EtherNode\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the ethernode app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add ethernode app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("EtherNode-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  ethernode-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("EtherNode-Qt"));
