...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeEtherNodeAmount"))
        settings.setValue("nAnonymizeEtherNodeAmount", 1000);
    nAnonymizeEtherNodeAmount = settings.value("nAnonymizeEtherNodeAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeEtherNodeAmount"))
        SoftSetArg("-anonymizeethernodeamount", settings.value("nAnonymizeEtherNodeAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeEtherNodeAmount:
            return QVariant(nAnonymizeEtherNodeAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeEtherNodeAmount:
            nAnonymizeEtherNodeAmount = value.toInt();
            settings.setValue("nAnonymizeEtherNodeAmount", nAnonymizeEtherNodeAmount);
            emit AnonymizeEtherNodeAmountChanged(nAnonymizeEtherNodeAmount);
            break;
        default:
            break;
        }
