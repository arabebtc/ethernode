...
int nAnonymizeEtherNodeAmount = 1000;
...
#else
    const char* pszModule = "ethernode";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\EtherNode
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\EtherNode
    // Mac: ~/Library/Application Support/EtherNode
    // Unix: ~/.ethernode
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "EtherNode";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "ethernode.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "ethernoded.pid"));
